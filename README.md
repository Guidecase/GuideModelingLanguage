...
+ **diagnose**
  ...
  BLOCK: yes  
  PARAMETERS: diagnosis name key  
  NOTES: A diagnosis is a probable illness or injury associated with one or more possible symptoms (see below). Multiple diagnoses will usually be defined for most guides. The diagnosis name key text is displayed to the user in the triage UI. If no symptoms are given for a diagnosis, that diagnosis will be considered the default if a user has no other matching diagnoses symptoms.
  EXAMPLE:
...
+ **low_risk**
  ...
  BLOCK: no  
  PARAMETERS: none  
  NOTES: If a diagnosis is marked as `low risk`, then it will never be displayed to the user as the most likely diagnosis for the triage, even if the user has more answers matching symptoms of this diagnosis than any other.  
  EXAMPLE:  
